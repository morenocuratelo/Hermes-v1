...
class HistoryManager:
    ...
    def push_state(self, data: Any) -> None:
        blob = pickle.dumps(data)  # noqa: S301
    ...
    def _load_entry(self, entry: dict) -> Any | None:
        try:
            if entry["type"] == "ram":
                return pickle.loads(entry["blob"])  # noqa: S301
            else:
...
class IdentityLogic:
    ...
    def merge_logic(self, master: int, slave: int) -> None:
        with self.lock:
            ...
            z = sorted(
                zip(
                    self.tracks[master]["frames"],
                    self.tracks[master]["boxes"],
                    strict=False,
                ),
                key=lambda x: x[0],
            )
    ...
    def split_track(self, track_id: int, split_frame: int, keep_head: bool) -> tuple[int | None, Any]:
        with self.lock:
            ...
            if keep_head:
                ...
                self.tracks[new_id] = {
                    ...
                    "boxes": tail_boxes,
                    "role": "Ignore",
                    "merged_from": inherited_merged,
                }
                self.id_lineage[new_id] = new_id
                self._log_operation(
                    "Split Track",
                    {"original": track_id, "new": new_id, "split_frame": split_frame, "kept": "head"},
                )
                ...
                self._log_operation(
                    "Split Track",
                    {"original": track_id, "new": new_id, "split_frame": split_frame, "kept": "tail"},
                )
    ...
    def auto_stitch(self, lookahead: int, time_gap: float, stitch_dist: float) -> int:
        with self.lock:
            ...
            self._log_operation(
                ...
                {
                    "merged_count": merged,
                    "params": {"lookahead": lookahead, "time_gap": time_gap, "stitch_dist": stitch_dist},
                },
            )
            return merged
    ...
    def absorb_noise(self, cast: dict[str, dict], noise_dist: float, time_gap: float) -> int:
        with self.lock:
            ...
            while changed:
                ...
                for main_id in list(main_tracks):
                    ...
                    for cand_id in candidates:
                        if cand_id not in self.tracks or not set(main_frames).isdisjoint(
                            self.tracks[cand_id]["frames"]
                        ):
                            continue
            ...
            self._log_operation(
                "Absorb Noise",
                {"absorbed_count": absorbed, "params": {"noise_dist": noise_dist, "time_gap": time_gap}},
            )
            return absorbed

    def get_track_at_point(self, frame: int, x: int, y: int) -> int | None:
...
class IdentityView:
    def __init__(self, parent: tk.Widget, context: Any):
        ...
        if self.context.cast:
            ...
            self.cast = {
                ...
                "Confederate_2": {"color": (255, 0, 0)},  # noqa: S311
            }
    ...
    def _setup_ui(self) -> None:
        ...
        tk.Button(
            row1,
            text="⚡ Auto-Stitch",
            command=self.auto_stitch,
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        tk.Button(
            row1,
            text="🧹 Absorb Noise (Gap Fill)",
            command=self.absorb_noise_logic,
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        ...
        tk.Button(
            row2,
            text="🔗 Merge ALL by Role",
            bg="#d1e7dd",
            command=self.merge_all_by_role,
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
    ...
    def perform_undo(self, event: tk.Event | None = None) -> None:
    ...
    def _perform_autosave(self) -> None:
        ...
        data_to_save = {"tracks": tracks, "id_lineage": lineage, "audit_log": audit_log}
    ...
    def _check_for_autosave(self) -> None:
        ...
        if os.path.exists(path):
            if messagebox.askyesno(
                "Recovery",
                "Found an autosave file from a previous session.\nDo you want to restore it?",
            ):
                try:
                    with open(path) as f:
    ...
    def absorb_noise_logic(self) -> None:
        ...
        if not messagebox.askyesno(
            "Confirm",
            f"Absorb noise (Dist < {self.param_noise_dist}px)?" " This cannot be undone in one step.",
        ):
            return
        self._snapshot()
    ...
    def auto_stitch(self) -> None:
        ...
        if not messagebox.askyesno(
            "Confirm",
            "Run auto-stitching? This may merge unrelated tracks" " and cannot be undone in one step.",
        ):
            return
        self._snapshot()
        ...
        messagebox.showinfo(
            "Info",
            f"Stitched {merged} fragments (Lookahead:{p_win}," f" Time:{p_time}s, Dist:{p_dist}px).",
        )
    ...
    def split_track_at_current_frame(self, track_id: int | None = None, override_frame: int | None = None) -> None:
        ...
        msg = (
            f"Splitting track {track_id_to_split} at frame {split_frame}.\n\n"
    ...
    def refresh_tree(self) -> None:
        ...
        for n in self.cast:
            ...
            self.tree.tag_configure(n, background=f"#{min(r+180,255):02x}{min(g+180,255):02x}{min(b+180,255):02x}")
    ...
    def load_data_direct(self, video_path: str | None, json_path: str | None) -> None:
        if video_path:
    ...
    def save_mapping(self) -> None:
        ...
        id_frame_map: dict[int, list[tuple[int, set[int]]]] = {}
        ...
        try:
            with (
                gzip.open(self.json_path, "rt", encoding="utf-8") as f_in,
                    ...
                    try:
                        ...
                        if "det" in frame_data:
                            for i, det in enumerate(frame_data["det"]):
                                ...
                                lookup_id = tid  # noqa: S311
                                if tid == -1:
                                    lookup_id = 9000000 + (f_idx * 1000) + i  # noqa: S311
        ...
        messagebox.showinfo(
            "Done",
            f"Mapped {count} IDs.\n\nFiles Saved:\n"
            f"1. Identity Map: {os.path.basename(out)}\n"
            f"2. Enriched Data: {os.path.basename(enriched_out)}",
        )
    ...
    def refresh_cast_list(self) -> None:
        ...
        for n in self.cast:
            ...
            b, g, r = self.cast[n]["color"]
            self.list_cast.itemconfig(self.list_cast.size() - 1, bg=f"#{r:02x}{g:02x}{b:02x}")

    def add_person(self) -> None:
        n = simpledialog.askstring("New", "Name:")
        if n:
            self.cast[n] = {"color": (random.randint(50, 200), random.randint(50, 200), random.randint(50, 200))}  # noqa: S311
            self.refresh_cast_list()
        ...
        if i:
            ...
            for p in self.cast:
                self.context_menu.add_command(
                    label=f"Assign to {p}",
                    command=lambda n=p: self.assign_role_to_selection(n),  # type: ignore
                )

            self.context_menu.add_separator()
            self.context_menu.add_command(
                label="Remove Assignment",
                command=lambda: self.assign_role_to_selection("Ignore"),
            )
            self.context_menu.add_separator()
            ...
            try:
                ...
                pass  # Track access may fail for deleted tracks

            if can_split:
                self.context_menu.add_command(
                    label=f"✂️ Split at Frame {frozen_frame_for_split}",
                    command=lambda: self.split_track_at_current_frame(
                        track_id=tid,
                        override_frame=frozen_frame_for_split,
                    ),
                )
            else:
                self.context_menu.add_command(
                    label=f"✂️ Split (Frame {frozen_frame_for_split} out of bounds)", state="disabled"
                )
    ...
    def show_frame(self) -> None:
        ...
        for tid, d in self.logic.tracks.items():
            if self.current_frame in d["frames"]:
                ...
                cv2.putText(
                    frame,
                    f"{tid} {role if role!='Ignore' else ''}",
                    (x1, y1 - 5),
