...
class FilterView:
    ...
    def _setup_ui(self):
        tk.Label(self.parent, text="6. Gaze Filters (I-VT)", font=("Segoe UI", 18, "bold"), bg="white").pack(pady=(0, 10), anchor="w")
        ...
        ttk.Combobox(self.f_type, textvariable=self.tracker_type, values=["Wearable", "Screen-Based"], state="readonly", width=15).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(self.f_wearable, text="Tobii Glasses 3 Defaults:", fg="gray", bg="white").pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Combobox(f_pre, textvariable=self.noise_reduction, values=["None", "Moving Median", "Moving Average"], state="readonly", width=15).pack(side=tk.LEFT)
        ...
        tk.Checkbutton(f_post, text="Discard Short", variable=self.discard_short, bg="white").pack(side=tk.LEFT, padx=(15,0))
        ...
        # 4. Run
        self.btn_run = tk.Button(main, text="RUN I-VT FILTER", bg="#007ACC", fg="white", font=("Bold", 12), height=2, command=self.run_process)
