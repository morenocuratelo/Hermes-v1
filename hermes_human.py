...
class TextRedirector(object):
    ...
    def write(self, str):
        try:
            ...
            # Gestisce il caso in cui il widget viene distrutto chiudendo l'app
            pass
...
class YoloView:
    def __init__(self, parent, context):
        self.parent = parent    # Il Frame dentro hermes_unified
        self.context = context  # Il cervello condiviso (AppContext)
        
        # Variabili Locali
        self.video_path = tk.StringVar()
        self.output_path = tk.StringVar()
        self.model_name = tk.StringVar(value="yolo26x-pose.pt") # Aggiornato a v11 se disponibile, o usa v8/11
        self.is_running = False
        
        # --- SYNC CONTEXT (LETTURA) ---
        # Se un video è già stato caricato in un altro tab, lo recuperiamo qui
    ...
    def _build_ui(self):
        ...
        # 1. Hardware Info (Visualizzazione)
        ...
        tk.Label(lf_conf, text="(Il download del modello è automatico al primo avvio)", fg="gray", bg="white").pack(side=tk.LEFT)
        ...
        # Redirezione stdout (Cattura i print di YOLO)
    ...
    def _check_hardware_from_context(self):
        # Usiamo i dati già calcolati nel Context
    ...
    def _suggest_output_name(self, video_path):
        ...
        # Pre-impostiamo anche il percorso nel context per il futuro
    ...
    def run_yolo_process(self):
        ...
        try:
            ...
            # Aggiorna progress bar (thread safe call)
            self.parent.after(0, lambda: self.progress.configure(maximum=total_frames))
            
            print("Avvio tracking...")
            
            # Esecuzione YOLO
            # device=self.context.device forza l'uso di quello che abbiamo rilevato
            results = model.track(
                source=video_file,
                stream=True,
                persist=True,
            ...
            print(f"✅ COMPLETATO! Output: {out_file}")
            
            # Aggiorna context finale
