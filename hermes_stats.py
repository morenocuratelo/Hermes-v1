...
class StatsLogic:
    ...
    def export_master_report(self, output_path, data_frames_dict):
        ...
        _kp_cols = [
            f"{kp}_{axis}" for kp in _kp_names
            for axis in ["X", "Y", "C"]
        ]
        _kp_descs = [
            f"{kp}: {desc}" for kp in _kp_names
            for desc in [
        ...
        legends_dict = {
            ...
            "Enriched": pd.DataFrame({
                "Column": [
                    "Frame", "Timestamp", "TrackID", "MasterID",
                    "Role", "Conf",
                    "Box_X1", "Box_Y1", "Box_X2", "Box_Y2",
                ] + _kp_cols,
                "Description": [
            ...
            "AOI": pd.DataFrame({
                "Column": [
                    "Frame", "Timestamp", "TrackID", "Role",
                    "AOI", "ShapeType", "ShapePoints",
            ...
            "Mapping": pd.DataFrame({
                "Column": [
                    "Timestamp", "Frame_Est", "Gaze_X", "Gaze_Y",
                    "Hit_Role", "Hit_AOI", "Hit_TrackID", "Hit_Shape",
                    "Hit_x1", "Hit_y1", "Hit_x2", "Hit_y2",
                    "Raw_Gaze2D_X", "Raw_Gaze2D_Y",
                ],
                "Description": [
                    "Timestamp in secondi dall'inizio del video.",
            ...
            "Stats_Summary": pd.DataFrame({
                "Column": [
                    "Name", "Start", "End", "Duration",
                    "Trial", "Condition", "Phase",
                    "Gaze_Samples_Total", "Gaze_Valid_Time",
                    "Tracking_Ratio", "Hit_Role", "Hit_AOI",
                    "Glances", "Latency", "Percentage",
                    "Avg_Glance_Dur", "Fixation_Dur",
                    "Fixation_Count", "TTFF",
                ],
                "Description": [
                    "Nome univoco del TOI/fase sperimentale.",
...
class GazeStatsView:
    ...
    def run_analysis_thread(self):
        ...
        if freq is not None and freq <= 0.0: # type: ignore
            freq = None
